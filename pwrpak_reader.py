    ...
    with open (fname) as csvfile:
        ...
        for row in pclreader:
            if ( '$--GPGGA' in row):
                ...
                cnt[1] += 1
            elif ('#CORRIMUDATAA' in row):
                point = point0.copy()
                # week seconds lat lon alt Nvel Evel Uvel Roll Pith Azim 
                week = int(row[5])
                seconds = float(row[6])#.split(';')[0])
                ts = week_seconds_to_utc(week,seconds,0)                
                #print (row[0] + " " + str(ts))
                _imu = imu0.copy()
                _imu['time'] = ts
                
                _imu['accY'] = float(row[15])
                _imu['accX'] = float(row[14])
                _imu['accZ'] = float(row[16].split('*')[0])
                imu.append(_imu)
